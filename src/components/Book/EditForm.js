...
const EditForm = (props) => {
    ...
    const editBook = (id, name, category, authorId, availableCopies) => {
        BookService.edit(id, name, category, authorId, availableCopies)
    ...
    useEffect(() => {
        ...
        BookService.findById(bookParamsId.id).then((data) =>{
            setBook(data.data)
            setFormData((prevState => ({
                ...
                <select name={"authorId"} onChange={handleChange} className={"form-control"}>
                    {authors.map((value, index) => {
                        if (value.id === book.author.id)
                            return <option selected key={index} value={value.id}>{value.name + ' ' + value.surname}</option>
                        else
